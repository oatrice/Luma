...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Luma: Multi-Agent Coding Assistant")
    parser.add_argument("--task", type=str, help="Coding task description")
    parser.add_argument("--file", type=str, help="Target file name (optional)")
    parser.add_argument("--github", action="store_true", help="Enable GitHub integration (deprecated, integrated into flow)")
    parser.add_argument("--source", type=str, nargs="+", help="List of source files to provide as context")
    parser.add_argument("--disable-log-truncation", action="store_true", help="Disable log truncation for Tester Agent")
    parser.add_argument("--docs-only", action="store_true", help="Skip coding, only update documentation (CHANGELOG/Version)")
    ...
    if args.task:
        ...
        filename_input = args.file if args.file else ""
        repo_input = "oatrice/Tetris-Battle"
        ...
        print("🚀 Luma Agent Started!")
        task_input = input("Enter coding task: ").strip()
        filename_input = input("Enter target filename (optional): ").strip()
        repo_input = input("Enter GitHub Repo (e.g. oatrice/Tetris-Battle): ").strip() or "oatrice/Tetris-Battle"
    ...
    initial_state = AgentState(
        ...
        iterations=0,
        approved=False,
        disable_log_truncation=args.disable_log_truncation,
        changes={},
        source_files=args.source if args.source else [],
        repo=repo_input,
        issue_data={},
        test_suggestions="",
        skip_coder=args.docs_only
    )
    
    print(f"\n▶️ Starting Workflow with task: '{task_input}'\n")

    try:
        if args.task or not sys.stdin.isatty():
             # CLI/Pipe Mode: Direct Invoke
             final_state = app.invoke(initial_state)
