...
def publisher_agent(state: AgentState):
    ...
    # VCS tools are imported conditionally, but we check platform-specific availability below
    pass
    ...
    if current_branch and current_branch not in ["main", "master"]:
        ...
        subprocess.run(
            ["git", "checkout", "-b", branch_name], cwd=target_dir, capture_output=True
        )
    else:
    ...
    if feature_id:
        ...
        if matches:
            ...
            if not all_screens and not state.get("auto_approve", False):
                ...
                print(
                    "   Please add screenshots now to help the LLM write a better PR description."
                )
                inp = (
                    ui.safe_input(
                        "   Press Enter after adding screenshots (or 's' to skip)..."
                    )
                    .strip()
                    .lower()
                )
...
"""
        ...
        if os.path.exists(manual_body_path) and os.path.getsize(manual_body_path) > 0:
            ...
            try:
                ...
                if issue_url and f"Closes {issue_url}" in existing_body:
                    print(
                        f"🤖 Auto-Approve: Draft body correctly targets issue URL ({issue_url}). Using manual PR body..."
                    )
                    use_existing = True
                else:
                    print(
                        f"⚠️ Auto-Approve: Draft body does NOT match current issue URL ({issue_url}). Regenerating..."
                    )
            except Exception as e:
        ...
        if choice == "y":
            ...
            try:
                ...
                if auto_approve:
                    ...
                    confirm = (
                        ui.safe_input("👉 Submit this PR description? (y/N): ")
                        .strip()
                        .lower()
                    )
                ...
                if confirm == "y":
                    ...
                    if auto_approve:
                        ...
                        return {}
                    print(
                        "🔄 Cancelled submission. Returning to menu (Select 'm' to use draft later)."
                    )
            ...
            if os.path.exists(manual_body_path):
                try:
                    ...
                    if auto_approve:
                        ...
                        confirm = (
                            ui.safe_input(
                                "👉 Submit this manual PR description? (y/N): "
                            )
                            .strip()
                            .lower()
                        )
                    ...
                    if confirm == "y":
                        ...
                        print(
                            "🔄 Cancelled submission. You can edit the file and try again."
                        )
                        choice = ""
                        continue
                except Exception as e:
    ...
    try:
        ...
        if verify.returncode != 0:
            raise Exception(
                f"Remote branch 'origin/{branch_name}' not found after push!"
            )
