...
def create_merge_request(repo_name, title, body, source_branch, target_branch="main"):
    ...
    try:
        ...
        for line in lines:
            if "http" in line and (
                "merge_requests" in line or "-/merge_requests" in line
            ):
                return line.strip()
...
def get_open_merge_request(repo_name, source_branch):
    ...
    try:
        ...
        # Log the exact command that failed for debugging
        ...
        if "404" in e.stderr and "Not Found" in e.stderr:
            ...
            print(
                f"   Failed to check open GitLab MRs: Command {' '.join(e.cmd)} returned {e.returncode}"
            )
...
def update_merge_request(repo_name, mr_number, title=None, body=None):
    ...
    try:
        ...
        # Log the exact command that failed for debugging
        print(
            f"   Failed to update GitLab MR: Command {' '.join(e.cmd)} returned {e.returncode}"
        )
        print(f"   Stderr: {e.stderr}")
        print(f"   Stdout: {e.stdout}")
        return None
